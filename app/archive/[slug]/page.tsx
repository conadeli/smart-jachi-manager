    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ background: bg, borderRadius: 12, padding: '1.25rem' }}>
          ...
          {result.legalBasis ? (
            <div style={{ fontSize: 11, color, background: 'rgba(255,255,255,0.6)', padding: '3px 8px', borderRadius: 6, display: 'inline-block' }}>📚 {result.legalBasis as string}</div>
          ) : null}
        </div>
        {result.negotiation ? (
          <div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.5rem', textAlign: 'center' }}>
        ...
        {result.averageRange ? <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 12 }}>이 지역 평균: {result.averageRange as string}</div> : null}
